...
| ID | What | Notes |
| --- | --- | --- |
| S-95 | The music library is duplicates and unmatched metadata | GitHub #16. Measured: **1,064 `content_hash` values appear more than once** — byte-identical files catalogued twice or more — and 1,722 titles appear more than once, including punctuation-only variants (`recognize (feat. riverkinn)` against `recognize (feat riverkinn)`) and one titled `#` ten times. **All 8,440 music rows carry `match_confidence = none`**, so nothing has ever matched a metadata source, which is why the titles are filenames. Deleting on a title match would destroy real files; the hash duplicates are the only ones provable without listening to them |
