...
| S-55 | Move a whole server to another machine — database, files and profiles | Plan written: `ServerTransfer.md`, ~6.5 days. 8,315 files, 46.3 GB, so resumability is the feature rather than polish. The receiver pulls over a tailnet or forwarded address using a Sanctum token from the source; every file is hashed before and after and a mismatch is deleted rather than kept. Measured before designing: media gzips 0.4% and the database 85%, so the database is compressed and the media is not |
