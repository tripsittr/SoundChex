...
| ID | What | Notes |
| --- | --- | --- |
| S-85 | Films and shows are not being classified correctly | Reported 22 Aug. Not yet diagnosed — no examples captured, so the first job is to find what is landing in the wrong type and whether it is the scanner, the episode parser or the metadata match that decides it |
