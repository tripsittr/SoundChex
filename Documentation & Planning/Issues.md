...
| ID | What | Notes |
| --- | --- | --- |
| S-101 | Playback specs wait on `currentTime`, which does not advance in this browser | Eleven desktop specs went red on the Mac — `player-session` (4), `playback` (2), `now-playing-sheet`, `library-refresh`, `connection-toast`, `download-queue`, `failover` — having been 184/184 green earlier the same day. Not a regression: the same spec fails at `421c97a`, the commit that produced that green run, and the specs themselves are four days old and unchanged. Probed the element directly: `paused: false`, `readyState: 4`, `networkState: 1`, `error: null`, and `currentTime` frozen at **exactly 0.02322** after 8 seconds, identical across runs. The player, the stream and the fixture are all fine — the fixture is a clean 30s mono MP3 that decodes without error under ffmpeg, and the file is served over a live signed-in session. The browser is simply not advancing the media clock. `--use-fake-device-for-media-stream` and `--autoplay-policy=no-user-gesture-required` changed nothing (those cover `getUserMedia`, not `<audio>`), so that attempt was reverted rather than left in. `startPlaying()` in `player-session.spec.js:23` polls for `currentTime > 6` within 15s, which cannot be satisfied when the clock does not move; the assertion needs to be something a headless browser can honour — a `play` event, or `readyState`/`paused` — rather than wall-clock progress. Note the machine was also running two agent sessions and the full suite degraded 6.7 → 14 → 42.9 minutes over the same period, so contention is a plausible trigger for the audio clock being suspended, but that is unproven and is not the failure itself. |
...
| S-86 | Four PHP tests compare `/` against `\` and fail on Windows | Was six. `LibraryScannerTest`'s two were describing S-98 — a real bug, now fixed, and they are green. What remains is `LibraryOrganizerTest` (3) and `ConversionFilerTest` (1) asserting `media/library/...` while the organiser writes `media\library\...` there, plus `HostServicesTest` writing to a macOS-only log path. Still worth clearing: while the suite is red by default a real failure cannot be told from the noise |
